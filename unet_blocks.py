...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, in_channels, concat_layer_depth, filter_num, interpolate=False, dropout=False, dropout_rate=0.3):
        ...
        self.up_sample_interpolate = nn.Sequential(nn.Upsample(scale_factor=2,
                                                   ...
                                                   nn.Conv2d(in_channels=self.in_channels + self.concat_layer_depth,
                                                             out_channels=self.in_channels + self.concat_layer_depth,
                                                             kernel_size=3,
                                                             padding=1)
                                                   )
    ...
    def forward(self, x, skip_layer, seeds=None, upsample=True):
        ...
        merged_out = torch.cat([x, skip_layer], dim=1)
        if upsample is True:
            up_sample_out = F.leaky_relu(self.up_sample_interpolate(merged_out))
        else:
            up_sample_out = merged_out
        out = self.down_sample(up_sample_out, seeds=seeds)
        return out
