...
class UNetEncoder(nn.Module):
    ...
    def forward(self, x, seeds=None):
        ...
        for stage, enc_op in enumerate(self.contracting_path):
            ...
            if stage < len(self.contracting_path) - 1:
...
class UNetDecoder(nn.Module):
    """
     ...
     Returns:
         out (torch.Tensor) : Prediction of the segmentation map
     """
    def __init__(self, n_channels=1, base_filter_num=64, num_blocks=4, num_classes=5, mode='2D', dropout=False, dropout_rate=0.3, use_pooling=True, enc_outputs=[]):
        ...
        self.n_classes = int(num_classes)
        self.base_filter_num = int(base_filter_num)
        self.enc_layer_depths = [32]  # Keep track of the output depths of each encoder block
        self.mode = mode
        self.pooling = use_pooling
        self.dropout = dropout
        self.dropout_rate = dropout_rate
        ...
        # Bottleneck layer
        if self.mode == '2D':
            bottle_neck_filter_num = self.enc_layer_depths[-1]
            bottle_neck_in_channels = self.enc_layer_depths[-1]
            self.bottle_neck_layer = self.encoder(filter_num=bottle_neck_filter_num,
                                                  in_channels=bottle_neck_in_channels)
        ...
        for block_id in range(num_blocks):
            if self.dropout is True and block_id < 2:
                ...
                self.expanding_path.append(self.decoder(in_channels=dec_in_channels,
                                                        filter_num=self.enc_layer_depths[-1-block_id-1],
                                                        concat_layer_depth=self.enc_layer_depths[-1-block_id],
                                                        interpolate=False,
                                                        dropout=False))

            dec_in_channels = self.enc_layer_depths[-1-block_id-1]
    ...
    def forward(self, x, enc_outputs, seeds=None):
        ...
        for block_id, dec_op in enumerate(self.expanding_path):
            if block_id < 2:
                ...
                if block_id == len(self.expanding_path) - 1:
                    x = dec_op(x, enc_outputs[-1 - block_id], upsample=False)
                else:
                    x = dec_op(x, enc_outputs[-1-block_id])
